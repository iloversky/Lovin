...
function controlChar(objId, maxLen) {
	...
	jQuery("#tipText").remove();
	if(document.getElementById("tipText")) {
		document.getElementById("tipText").innerHTML = tip;
	} else {
		//jQuery(obj).after("<p style='color: red' id='tip-" + objId + "'>" + tip + "</p>");
		jQuery(obj).after("<p style='color: red' id='tipText'>" + tip + "</p>");
	}
	/*jQuery(obj).blur(function() {
		jQuery("#tip-" + objId.replace(/\./, "\\.")).remove();
	});*/
}
...
$(document).ready(function() {
	$(".notBlank").keyup(function() {
		// 去除前后空白符 
		if(/^\s/.exec($(this).val()) || /\s$/.exec($(this).val())) {
			$(this).val($(this).val().Trim());			
		}
	});
});

// 验证不能为空, obj是 id时, 且包含 .号, 需要写成[objId\\.id]的形式
